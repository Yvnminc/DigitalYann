...
SITE_URL = os.getenv('SITE_URL', 'https://digitalyann.app')
SITE_NAME = os.getenv('SITE_NAME', 'Digital Yann')
...
async def chat_completion(request: ChatRequest):
    ...
    try:
        # Process the request to match OpenRouter format
        # Convert any string content to list format if needed
        processed_messages = []
        
        system_instruction = '''
        <Role>
        Yann是一个AI老师，我希望你能够扮演他的角色，为学生指点迷津，你的任务是根据我知识进行回答，每个问题只要输出plain text，不要使用bullet point和subtitle，回答更关注当前问题的深度而不是广度，不要随意进行拓展。用户使用什么语言（中文，英文），你就用什么语言进行回复。
        </Role>
        ...
        '''
        # Add system message with the instruction
        system_message = {
            "role": "system",
